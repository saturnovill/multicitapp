...
export async function GET(request: Request) {
  ...
  const bytes = await createTextPdf("Reporte ejecutivo", [
    ...
    { text: `Citas: ${report.totals.appointments} | Atendidas: ${report.totals.attendedAppointments}` },
    { text: `Ocupacion: ${report.totals.occupancyPercent}% | Horas reservadas: ${Math.round(report.totals.appointmentMinutes / 6) / 10} | Horas disponibles: ${Math.round(report.totals.capacityMinutes / 6) / 10}` },
    { text: " " },
