...
type Setup = {
  ...
  cashSessions: { id: string; branchId: string }[];
  giftCards: { id: string; code: string; balanceCents: number; expiresOn: string | null }[];
};

type Line = { key: string; serviceId: string; employeeId: string; quantity: number };
type Payment = { method: "cash" | "card" | "transfer" | "giftcard"; amount: string; reference: string; giftCardId: string };
...
export function SaleForm({ setup, initialAppointmentId = "" }: { setup: Setup; initialAppointmentId?: string }) {
  ...
  return (
    <form action={action} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <input type="hidden" name="payments" value={JSON.stringify(payments.filter((payment) => Number(payment.amount) > 0).map((payment) => ({ method: payment.method, amountCents: Math.round(Number(payment.amount) * 100), reference: payment.reference, giftCardId: payment.giftCardId })))} />
      ...
      <div className="space-y-6 xl:sticky xl:top-6 xl:self-start">
        ...
        <Button className="w-full" size="lg" disabled={pending || !branchId || !hasOpenCashSession || lines.some((line) => !line.serviceId || !line.employeeId) || payments.some((payment) => payment.method === "giftcard" && !payment.giftCardId) || paid < total}>{pending ? "Registrando…" : "Cobrar y generar folio"}</Button>
      </div>
    </form>
  );
}
