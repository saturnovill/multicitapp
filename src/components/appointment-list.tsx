...
export type AppointmentFilters = { from?: string | string[]; to?: string | string[]; status?: string | string[]; branchId?: string | string[]; q?: string | string[]; page?: string | string[] };
...
export async function AppointmentList({ filters, companyId, employeeId, calendarBasePath, showCompany }: {
  ...
  const page = Math.max(1, Math.min(10_000, Number.parseInt(single(filters.page) || "1", 10) || 1));
  const pageSize = 50;
  ...
  if (q) query.set("q", q); if (validDate(from)) query.set("from", from); if (validDate(to)) query.set("to", to); if (branchId) query.set("branchId", branchId); if (parsedStatus) query.set("status", parsedStatus);
  const pageHref = (target: number) => { const next = new URLSearchParams(query); next.set("page", String(target)); return `?${next}`; };

  return (
    <div className="space-y-5">
      <Card><CardContent className="pt-6"><form className="grid gap-3 md:grid-cols-6"><Input name="q" defaultValue={q} placeholder="Cliente o empleado" className="md:col-span-2" /><Input name="from" type="date" defaultValue={from} aria-label="Desde" /><Input name="to" type="date" defaultValue={to} aria-label="Hasta" /><select name="branchId" defaultValue={branchId ?? ""} className="h-9 rounded-md border border-input bg-transparent px-3 text-sm"><option value="">Todas las sucursales</option>{branchRows.map((branch) => <option key={branch.id} value={branch.id}>{showCompany ? `${branch.companyName} · ` : ""}{branch.name}</option>)}</select><select name="status" defaultValue={parsedStatus ?? ""} className="h-9 rounded-md border border-input bg-transparent px-3 text-sm"><option value="">Todos los estados</option>{statuses.map((status) => <option key={status} value={status}>{statusLabel[status]}</option>)}</select><div className="flex flex-wrap gap-2 md:col-span-6"><Button type="submit"><Search />Filtrar</Button><Button variant="outline" asChild><Link href="?">Limpiar</Link></Button><Button variant="outline" asChild><Link href={`/api/appointments.csv?${query}`}><Download />Exportar CSV</Link></Button></div></form></CardContent></Card>
      ...
      <div className="flex items-center justify-between gap-3 text-sm text-muted-foreground"><span>{totalRows} cita{totalRows === 1 ? "" : "s"} · Página {Math.min(page, totalPages)} de {totalPages}</span><div className="flex gap-2"><Button variant="outline" size="sm" asChild={page > 1} disabled={page <= 1}>{page > 1 ? <Link href={pageHref(page - 1)}>Anterior</Link> : <span>Anterior</span>}</Button><Button variant="outline" size="sm" asChild={page < totalPages} disabled={page >= totalPages}>{page < totalPages ? <Link href={pageHref(page + 1)}>Siguiente</Link> : <span>Siguiente</span>}</Button></div></div>
    </div>
  );
}
