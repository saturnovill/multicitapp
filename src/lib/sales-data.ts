...
export async function getSaleSetup(companyId: string) {
  const db = getDb();
  const [companyRows, branchRows, customerRows, serviceRows, employeeRows, appointmentRows] = await Promise.all([
  ...
  const appointmentServiceRows = appointmentIds.length
    ...
    : [];
  return { company: companyRows[0] ?? null, branches: branchRows, customers: customerRows, services: serviceRows, employees: employeeRows, appointments: appointmentRows, appointmentServices: appointmentServiceRows };
}
